          ...
          >
            {React.Children.map(children, (child, index) => (
              <Card
                key={index}
                data-testid="card"
                className="w-full overflow-hidden bg-white transition-shadow duration-300 hover:shadow-lg dark:bg-slate-800 md:w-fit md:min-w-[500px]"
              >
                <CardContent className="p-6">{child}</CardContent>
              </Card>
            ))}
          </div>
        </div>
